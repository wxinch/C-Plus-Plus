...
namespace selection_sort_recursive {
/**
 * @brief The main function finds the index of the minimum element(采用递归的方法，寻找最小值下标)
 * @tparam T type of array
 * @param in_arr array whose minimum element is to be returned
 * @param current_position position/index from where the in_arr starts(开始寻找的下标位置，默认值为0)
 * @returns index of the minimum element(返回最小值下标)
 */
template <typename T>
uint64_t findMinIndex(const std::vector<T> &in_arr,
                      uint64_t current_position = 0) {
    // 若pos为数组最后一个元素的下标，直接返回pos
    if (current_position + 1 == in_arr.size()) {
        return current_position;
    }

    // 递归，寻找[pos+1, n-1]范围内的最小值下标
    uint64_t answer = findMinIndex(in_arr, current_position + 1);

    // 将[pos+1, n-1]范围内的最小值与in_arr[pos]比较，可得到[pos, n-1]范围内的最小值下标
...
/**
 * @brief The main function implements Selection sort(该函数运行结束之后，能将[current_position, n-1]范围的元素排好)
 * @tparam T type of array
 * @param in_arr array to be sorted,
 * @param current_position position/index from where the in_arr starts(排序范围为[current_position, n-1])
 * @returns void
 */
template <typename T>
void selectionSortRecursive(std::vector<T> &in_arr,
    ...
    // 寻找[current_position, n-1]范围内的最小值下标
    ...
    // 第二步：将[current_position+1, n-1]范围内的数组排好
