...
std::vector<uint64_t> selectionSort(const std::vector<uint64_t> &arr,
    ...
    // 每循环一次可以排好一个元素，共需要循环len-1次，[0, it-1]为已经排好的序列，it为待排序列的第一个元素
    for (uint64_t it = 0; it < len; ++it) {
        uint64_t min = it;  // set min value, 初始时假设最小值下标为it
        for (uint64_t it2 = it + 1; it2 < len; ++it2) {
            if (array[it2] < array[min]) {  // check which element is smaller，找到比array[min]更小的元素
                min = it2;  // store index of smallest element to min，更新最小值的下标
            }
        }

        if (min != it) {  // 如果找到的最小值下标与待排序列第一个元素下标it不同，则交换二者位置
